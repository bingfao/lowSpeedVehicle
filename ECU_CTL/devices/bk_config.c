...
int32_t bk_config_utc_sec_nsec_read(uint8_t *data)
{
    ...
    ret = driver_read(g_driver_bk_cfg, BK_CFG_UTC_SEC_NSEC_ADDR, read_data, BK_CFG_UTC_SEC_NSEC_SIZE + BK_CFG_CRC_16_SIZE);
    if (ret < 0) {
...
int32_t bk_config_utc_sec_nsec_write(uint8_t *data)
{
    ...
    *(uint16_t *)&write_data[BK_CFG_UTC_SEC_NSEC_SIZE] = crc16;
    ret = driver_write(g_driver_bk_cfg, BK_CFG_UTC_SEC_NSEC_ADDR, write_data, BK_CFG_UTC_SEC_NSEC_SIZE + BK_CFG_CRC_16_SIZE);
    if (ret < 0) {
